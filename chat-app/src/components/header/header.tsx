...
export default function Header() {
  ...
  const [search, setSearch, searchData, addToSearchData] = useStore((state) => [
    state.search,
    state.setSearch,
    state.searchData,
    state.addToSearchData,
  ]);
  ...
  const selectUser = async (id: string) => {
    if (id !== localStorage._id) {
      const data = await createRoom(id);
  ...
  const filterUsers = () => {
    return searchData?.filter((item) => item._id !== localStorage._id);
  };
  ...
  return (
    <div className="header">
      ...
      <div className="user-settings">
        ...
        <img className="user-profile" src={localStorage.pic} alt="" />
      </div>
    </div>
  );
}
